 ...
 * [Updated]
 * 
 * - Added Shell function for getting file
 *   extension after getting some commet
 *   about it, check link below.
 *   (http://www.codeguru.com/Csharp/Csharp/cs_misc/icons/article.php/c4261/)
 * - IconFromResource was added for having
 *   all the common needed functions for 
 *   handling icons.
 * - Readded DestroyIcon and adding GetManagedIcon,
 *   after verifying it's needed
 *   (thanks DrGui and Kenneth Broendum).
 * - Removed the IconFromExtension that pulled
 *   the icon from the registry
 *  (IconFromExtensionShell is default now).
 * - Fixed IconCount mistake
 *   (if the number of icon in the file was 0 and the
 *   index that was selected was also 0
 *   the ExtractIconEx still got to run and
